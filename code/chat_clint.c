...
int main(int argc, char *argv[])
{
	...
	# 첫 번째 인자 : &snd_thread(전송 쓰레드), 두 번째 인자 : NULL(기본 특성 사용), 세 번째 인자 : send_msg(사용할 쓰레드 함수), 네 번쨰 인자 : &sock(send_msg의 인자)
...
void *send_msg(void *arg)
{
	...
	{  
		...
		# 첫 번째 인자 : sock(클라이언트 소켓), 두 번쨰 인자 : name_msg(보낼 데이터),  세 번째 인자 : strlen(name_msg)(데이터의 크기) 
