...
int main(int argc, char &argv[])
{
	...
	while(1)
	{
		...
		# 첫 번째 인자 :  serv_sock(서버 소켓), 두 번째인자 : &clnt_adr(클라이언트 주소), 세 번쨰 인자 : 클라이언트 주소의 길이 
...
void *handle_clnt(void *arg)
{
	...
	# 첫 번쨰 인자 : clnt_sock(클라이언트 소켓), 두 번째 인자 : msg(읽어들인 데이터를 저장할 버퍼), 세 번째 인자 : sizeof(msg)(버퍼의 크기)
